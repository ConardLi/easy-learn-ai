...
rg "想象一下|让我们|其实只是|不是.*而是|它只是|从不|根本不" src/components/Section{Hero,Opening,Live,DarkKnowledge}*.tsx

# Hero H1 是否是「XXX 是什么？」格式（应能匹配到）
rg "是什么？" src/components/Section{Hero,Opening,Live,DarkKnowledge}*.tsx
