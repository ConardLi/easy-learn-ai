 ...
 * 添加了 CORS 处理和更详细的错误信息
 */

import { DailyListResponse } from '../types';
...
export const apiUtils = {
  ...
  async fetchDailyList(): Promise<DailyListResponse> {
    try {
      const response = await fetch(`${BASE_URL}/home/home.json`, {
        method: 'GET',
        mode: 'cors',
        headers: {
  ...
  async fetchDailyContent(date: string): Promise<string> {
    try {
      ...
      console.log(`Successfully fetched daily content for ${date}`);
      return data;
    } catch (error) {
