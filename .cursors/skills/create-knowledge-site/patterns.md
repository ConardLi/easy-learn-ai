...
**前提**：交互的唯一价值是**让用户摸到概念**。无关教学的炫技交互不如一个静态 SVG + 准确文字。

---

## 交互密度分级

判断 section 可动元素落在某层。**整站基线：≥ 3 个 section 到 L3+ + 出现 ≥ 4 种不同范式**（达不到说明这站太"读"，加 1-2 个让用户动手的）。但**单 section 不强制 2 个可动元素** —— 概念本身是"看一眼就懂"的就别硬塞。
...
**避免**：pill→pill / trace→trace / slider→slider / 静态图→静态图 连续出现。用户会审美疲劳。

整站 6-8 个 section 应出现 **4+ 种不同范式**。出现 3+ 个 pill 或 3+ 个 trace → 停下重排。

> 例外：本概念的教学路径**确实**就是连续两个对比（A vs B, then B vs C），那 pill→pill 也允许 —— 但视觉样式要拉开（一个横排一个竖排 / 配色不同 / 卡片样式不同）。
